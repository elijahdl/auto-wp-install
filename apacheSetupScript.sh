...
mysql -u root -p -e "CREATE DATABASE $wpDatabaseName; CREATE USER $wpDatabaseUser@localhost; SET PASSWORD FOR $wpDatabaseUser= PASSWORD('$wpDatabasePassword'); GRANT ALL PRIVILEGES ON $wpDatabaseName.* TO $wpDatabaseUser IDENTIFIED BY '$wpDatabasePassword'; FLUSH PRIVILEGES; "
